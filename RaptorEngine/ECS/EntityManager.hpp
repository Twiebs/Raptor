...
public:
	...
	T* AddSystem(Args... args) {
		static_assert(std::is_base_of<ISystem, T>::value, "Template paramater is not a system!");
		T* system = new T(args...);
		system->AddedToManager(this);
		system->Startup(this);
		//system->entities = GetEntities(system->componentBits);
...
private:
	...
	uint64 nextEntityID   =	1;	//The entity id is the index of the created entitiy.  These are recycled
	uint64 nextEntityUUID = 1;	//This is a unique identifier everytime a entitiy is recycled or created
	...
	std::vector<Entity*> removedEntities;	//Array of entities that have been removed and are awaiting recylcing
...
public:
	...
	uint64 systemsAdded;		//Count of all the systems ever added to the engine
	uint64 systemsRemoved;		//Count of all the systems ever removed from the engine
