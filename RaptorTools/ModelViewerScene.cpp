...
void ModelViewerScene::OnLoad(Engine& engine) {
	...
	this->engine = &engine;
	model = ResourceManager::LoadModel("Resources/models/nanosuit/nanosuit.obj");
	shader = std::make_unique<GLSLProgram>("Resources/shaders/Model.vert", "Resources/shaders/Model.frag");

	camera = std::make_unique<PerspectiveCamera>(engine.GetApp()->GetWidth(), engine.GetApp()->GetHeight());
...
bool ModelViewerScene::OnMouseButtonEvent(int button, bool isPressed, int mods){
	if (button == MouseButton::BUTTON_RIGHT){
		if (isPressed){
			engine->GetApp()->SetCursorHidden(true);
			isRotating = true;
			return true;
		}
		else{
