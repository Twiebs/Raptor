...
public:
	...
	T* CreateComponent(EntityID entityID, Args... args) {
		...
		componentsByEntityID[componentType->index].Set(entityID, componentIndex);
		return component;
	}
	...
	T* GetComponent(EntityID id) {
		...
		ComponentBlock* componentBlock = componentBlocks[componentType->index];
		assert(componentBlock != nullptr);
		uint32 componentIndex = componentsByEntityID[componentType->index].Get(id);
		if (componentIndex == 0) {
	...
	void RegisterComponent(uint64 flags) {
		...
		componentTypes.emplace_back(ComponentType{ flags, nextComponentTypeBit, nextComponentTypeIndex, sizeof(T) });
		...
		assert(block == nullptr);	//Block was already registered with the engine!
	...
	ComponentType* ComponentTypeOf() {
		...
		assert(type != nullptr); //Component types must be registered before use!
		return type;
	}
